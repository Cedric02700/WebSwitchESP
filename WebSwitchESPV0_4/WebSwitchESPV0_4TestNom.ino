...
unsigned long timerTimeoutDome = 0; 
boolean tempo13IsRunning = false;
boolean tempo14IsRunning = false;
boolean timeoutDomeIsRunning = false;
    ...
    while (client.connected())              //Tant que le client est connecté
    {
      ...
      { //Si des caracteres sont dispo
        ...
        for (int i = 0; i < nbRelais; i++)       //Gere les requetes client
        {
          ...
          {
            Serial.println("Debug: " + currentLine);
            if (etatRelais[i] == 0)
            {
              #ifdef NORMAL
              ...
              if(i == 12)
              {
                if(tempo14IsRunning == false)
                {
                  etatRelais[11] = 0;
                  digitalWrite(pinRelais[11], etatRelais[11]);
                  etatRelais[i] = 1 - etatRelais[i];
                  digitalWrite(pinRelais[i], etatRelais[i]);
                  tempo13Timer = millis(); 
                  tempo13IsRunning = true;
                  timerTimeoutDome = millis();
                  timeoutDomeIsRunning = true;  
                }              
              }
              if(i == 13)
              {
                if(tempo13IsRunning == false)
                {
                  etatRelais[11] = 0;
                  digitalWrite(pinRelais[11], etatRelais[11]);
                  etatRelais[i] = 1 - etatRelais[i];
...
void loop()
{
  ...
  if(tempo14IsRunning > 0)
  {
    if((millis() - tempo14Timer) > tempoDelay)
    {
      ...
      tempo14IsRunning = false;
      //Serial.println("fin de tempo 14");
    }
  }
  if(timeoutDomeIsRunning == true)
  {
    if((millis() - timerTimeoutDome) > timeoutDome)
    {
      etatRelais[11] = 1;
      digitalWrite(pinRelais[11], etatRelais[11]);
      timeoutDomeIsRunning = false;
    }
  }
  #endif
}
